...
// bash 层：tools/pre-execute 记录 bash 调用命令文本（callId 索引）；approval/request
//   瀑布监听器对 bash 沙箱升权请求做「路径代答」——命令原文（优先）或升权理由中
//   命中白名单路径（递归判定）即自动 allowed-once，未命中转人工（失败安全）。
...
/** 字面串出现且处于路径分量边界（其后是结尾/空白/引号/bash 元字符/分隔符）；防 /工作备份 误命中 /工作 */
function containsBounded(text, literal) {
  if (typeof text !== 'string' || text === '' || typeof literal !== 'string' || literal === '') return false
  const escaped = literal.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  return new RegExp(escaped + `(?:$|[\\s'"\\\`;|&<>()[\\]/])`).test(text)
}

/**
 * 文本 → 白名单判定：提取「/ 或 ~ 开头的路径样 token」，~ 展开 home 后
 * 逐个 canonical 解析并与白名单根做 isUnder 递归判定（填父目录即覆盖全部
 * 子目录，分量边界防 /工作备份 误命中 /工作）；另保留带分量边界的字面匹配
 * 兜底（v0.2 语义收严，覆盖 token 提取不到的边角）。
 * 失败安全：提取不到/未命中一律返回 false，转人工审批。
 */
export function textHitsWhitelist(text, roots) {
  if (typeof text !== 'string' || text === '' || roots.length === 0) return false
  const home = homedir()
  for (const root of roots) {
    if (containsBounded(text, root)) return true
    if (root === home || root.startsWith(home + sep)) {
      if (containsBounded(text, '~' + root.slice(home.length))) return true
    }
  }
  const tokens = text.match(/(?:~|\/)[^\s'"`;|&<>()[\]]+/g) ?? []
  for (const token of tokens) {
    const expanded = (token === '~' || token.startsWith('~/') || token.startsWith('~' + sep))
      ? home + token.slice(1)
      : token
    for (const root of roots) {
      if (isUnder(expanded, root)) return true
    }
  }
  return false
}

// bash 调用参数索引：callId → 命令文本。升权审批事件本身不带命令原文，只能查此表。
const MAX_INDEXED_CALLS = 200
const bashArgsByCallId = new Map()

function rememberBashArgs(callId, args) {
  if (typeof callId !== 'string' || callId === '') return
  if (bashArgsByCallId.size >= MAX_INDEXED_CALLS) {
    bashArgsByCallId.delete(bashArgsByCallId.keys().next().value) // 淘汰最早记录
  }
  bashArgsByCallId.set(callId, typeof args === 'string' ? args
...
export function apply(ctx) {
  ...
  // bash 层之一：记录 bash 调用命令文本（callId 索引），供升权代答查表
  ctx.effect(() => ctx.on('tools/pre-execute', (exec, next) => {
    if (exec?.name === 'bash') rememberBashArgs(exec.callId, exec.arguments)
    return next()
  }), 'fs-allowlist: bash call index')

  // bash 层之二：审批瀑布代答（仅 bash 的沙箱升权；命令原文/理由命中白名单 → allowed-once，否则转人工）
  // 第三参 true = {prepend:true}：unshift 到瀑布最外层，抢在官方 api-remotes 的 GUI 转发桥之前作答。
  // 该桥在 api-gateway.registerRemoteEvents 里启动即注册（早于一切 profile 插件），且只要 GUI 在线就
  // 认领请求并等用户点击后短路返回、几乎不调 next——不 prepend 的话本代答器在桌面会话永远轮空
  // （2026-09-18 排障：白名单路径升权仍弹卡，根因即此）。未命中照旧 next()，失败安全语义不变。
  ctx.effect(() => ctx.on('approval/request', (request, next) => {
    ...
    const commandText = bashArgsByCallId.get(request.callId) ?? ''
    // 命令原文优先、理由文本兜底：命令几乎必含目标路径，不再赌模型理由写不写路径
    if (!textHitsWhitelist(commandText, runtime.roots) && !textHitsWhitelist(reason, runtime.roots)) return next()
    ctx.logger.info(`[fs-allowlist] bash 升权代答：${(commandText || reason).slice(0, 140)}`)
    return 'allowed-once'
  }, true), 'fs-allowlist: bash approval answerer')
